...
class NiNDetailsScreenOne extends StatefulWidget {
  const NiNDetailsScreenOne({super.key});

  @override
  State<NiNDetailsScreenOne> createState() => _NiNDetailsScreenOneState();
}

class _NiNDetailsScreenOneState extends State<NiNDetailsScreenOne> {
    ...
    return AppScreenScaffold(
      ...
      bottomNavigationBar: AppBottomNavWrapper(
        child: AppPrimaryButton(
          text: "Proceed",
          onPressed: (ninNumberController.text.isEmpty || uploadedImagePath.isEmpty)
              ? null : () {
            context.read<KycViewModel>().captureNiNInfo({
              "type": "nin",
              "idNumber": ninNumberController.text,
              "documentImage": uploadedImagePath
            });
            AppNavigator.pushNamed(context, AppRoute.ninSelfieScreen);
          },
        ),
      ),
              ...
              child: Column(
                ...
                children: [
                  ...
                  AppInputField(
                    ...
                    validator: (value) {
                      if (value!.isEmpty) return 'NIN number is required';
                      return null;
                    },
                  ),
                ],
