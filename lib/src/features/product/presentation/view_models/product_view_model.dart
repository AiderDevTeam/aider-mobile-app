...
class ProductViewModel extends BaseViewModel with CanRent{
  ...
  UnmodifiableMapView<String, dynamic> get getProductDescription => UnmodifiableMapView(_productDescription);
  ...
  UnmodifiableMapView<String, dynamic> get getProductRequestBody => UnmodifiableMapView(_productRequestBody);

  void setModalWithCategory(CategoryModel categoryModel, [bool notify = false]){
  ...
  /// LIST A PRODUCT
  Future<void> listProduct(BuildContext context,{required Map<String, dynamic> requestBody}) async{
    ...
    if(context.mounted && requestBody['address'] != null &&  (requestBody['address']['placeId'] != null && requestBody['address']['placeId'].toString().isNotEmpty)){
      final locationDetails = await context.read<LocationViewModel>().fetchLocationDetails(requestBody['address']['placeId']);
      ...
      final address = {
        ...
        "city": locality.first.isEmpty ? requestBody['address']['originName'] : locality.first
      };

      requestBody["address"] = jsonEncode(address);
    }

    final result = await _productRepository.listProduct(requestBody: requestBody);
    ...
    result.fold((failure){
      ...
      AppDialogUtil.showSuccessAlert(context, 'Item listed', alignment: Alignment(0.0, HelperUtil.isIOS? -0.88: -0.90),);
      AppNavigator.pushNamed(context, AppRoute.listedProductsScreen, arguments: true);
  ...
  /// USER PRODUCTS
  void setUserProductHistory(ProductHistoryModel history, [bool append = false]){
    if(append){
  ...
  UnmodifiableListView<ProductModel> get getProducts => UnmodifiableListView(_userProductHistory.data);
  PaginationModel? get getProductMeta => _userProductHistory.meta;

  Future<void> fetchUserProducts(BuildContext context,{required Map<String, dynamic> queryParam, String loadingComponent = 'fetchProducts', String? nextPage}) async{
    setComponentErrorType = null;
    setLoading(true, component: loadingComponent);

    final result = await _productRepository.fetchUserProducts(nextPage: nextPage, queryParam: queryParam);
  ...
  /// RENTING
  Future<void> requestForItem(BuildContext context, String productExternalId, {required Map<String, dynamic> requestBody}) async{
    AppDialogUtil.loadingDialog(context);
    if(context.mounted &&  (context.read<ProductViewModel>().getPlaceId.isNotEmpty && context.read<ProductViewModel>().getPlaceId.isNotEmpty)){
      final locationDetails = await context.read<LocationViewModel>().fetchLocationDetails(context.read<ProductViewModel>().getPlaceId);
      // final locality = HelperUtil.getLocalityFromAddressComponents(locationDetails?.addressComponents?? []);
      final longAndLat = {
        "latitude": locationDetails?.geometry?['location']['lat']?? 0,
        "longitude": locationDetails?.geometry?['location']['lng']?? 0,
    ...
    final result = await _productRepository.requestForItem(productExternalId,requestBody: requestBody);
    if(context.mounted) {AppNavigator.pop(context);}
      ...
      AppDialogUtil.popUpModal(
        ...
        modalContent: SuccessModalContent(
          ...
          onButtonPressed: (){
            ...
            AppNavigator.of(context)..pop()..pushNamedAndRemoveUntil(AppRoute.homeScreen, (route) => false);
          },
        ),
      );
    });
  ...
  /// PRODUCT PHOTOS
  Future<bool> deleteProductPhoto(BuildContext context,{required String productExternalId, required Map<String, dynamic> requestBody}) async{
    AppDialogUtil.loadingDialog(context);
    final result = await _productRepository.deleteProductPhoto(productExternalId: productExternalId, requestBody: requestBody);
    if(context.mounted) {AppNavigator.pop(context);}
    ...
    result.fold((failure){
      ...
      final index = _userProductHistory.data.indexWhere((obj) => obj.externalId == productExternalId);
      if(index >= 0){
        final userProduct = _userProductHistory.data[index].copyWith(
          photos: List.from(_userProductHistory.data[index].photos?? [])..removeWhere((obj) => obj.id == requestBody['photoId'])
        );
      ...
      response = right;
      AppDialogUtil.showSuccessAlert(context, 'Photo deleted', alignment: Alignment(0.0, HelperUtil.isIOS? -0.88: -0.90),);
    });

    return response;
  }

  Future<void> addProductPhoto(BuildContext context,{required String productExternalId, required Map<String, dynamic> requestBody}) async{
    AppDialogUtil.loadingDialog(context);

    final imagePath = await MediaFileUtil.getMultipartFile(requestBody['photos[]']);
    requestBody['photos[]'] = [imagePath];

    final result = await _productRepository.addProductPhoto(productExternalId: productExternalId, requestBody: requestBody);
    if(context.mounted) {AppNavigator.pop(context);}
    ...
    result.fold((failure){
      ...
      AppDialogUtil.showSuccessAlert(context, 'Photo uploaded', alignment: Alignment(0.0, HelperUtil.isIOS? -0.88: -0.90),);
    });
  }

  /// DELETE PRODUCT
  Future<void> deleteProduct(BuildContext context, {bool fromCloset = false, String productExternalId = ''}) async{
    AppDialogUtil.loadingDialog(context);
    ...
    result.fold((left) {
      ...
      if(right){
        final history = _userProductHistory.copyWith(
            data: List.from(_userProductHistory.data)..removeWhere((obj) => obj.externalId == productExternalId)
        );
        _userProductHistory = history;
        notifyListeners();
  ...
  /// PRODUCT UPDATE
  Future<void> updateProduct(BuildContext context,{required String productExternalId, required Map<String, dynamic> requestBody}) async{
    AppDialogUtil.loadingDialog(context);
    ...
    final result = await _productRepository.updateProduct(productExternalId: productExternalId, requestBody: requestBody);
    if(context.mounted) {AppNavigator.pop(context);}
  ...
  Future<bool> deleteProductPrice(BuildContext context,{required String productExternalId, required Map<String, dynamic> requestBody}) async{
    AppDialogUtil.loadingDialog(context);

    final result = await _productRepository.deleteProductPrice(productExternalId: productExternalId, requestBody: requestBody);
    if(context.mounted) {AppNavigator.pop(context);}
    ...
    result.fold((failure){
      ...
      if(index >= 0){
        final newProduct = _userProductHistory.data[index].copyWith(
          prices: List.from(_userProductHistory.data[index].prices?? [])..removeWhere((obj) => obj.id == requestBody['productPriceId']),
        );
        List<ProductModel> products = List.from(_userProductHistory.data);
  ...
  /// VENDOR PRODUCTS
  void setVendorProductHistory(ProductHistoryModel history, [bool append = false]){
    if(append){
  ...
  UnmodifiableListView<ProductModel> get getVendorProducts => UnmodifiableListView(_vendorProductHistory.data);
  PaginationModel? get getVendorProductMeta => _vendorProductHistory.meta;

  Future<void> fetchVendorProducts(BuildContext context,{String? vendorExternalId, required Map<String, dynamic> queryParam, String loadingComponent = 'fetchVendorProducts', String? nextPage}) async{
    setComponentErrorType = null;
    if(nextPage == null){
    ...
    final result = await _productRepository.fetchVendorProducts(vendorExternalId: vendorExternalId, nextPage: nextPage, queryParam: queryParam);
  ...
  /// PRICE STRUCTURE
  UnmodifiableListView<PriceStructureModel>  get getPriceStructure => UnmodifiableListView(_priceStructure);
  ...
  void fetchPriceStructure(){
    SocketService().once('fetchPriceStructure', (data){
      ZLoggerService.logOnInfo('FETCHING PRICE STRUCTURE \n ---- $data ---- \n${DateTime.now()}');
      if(data != null) setPriceStructure = PriceStructureList.fromJson(data).list;
    });
  ...
  void fetchCategories(){
    SocketService().once('fetchCategories', (data){
      ZLoggerService.logOnInfo('FETCHING CATEGORIES \n ---- $data ---- \n${DateTime.now()}');
  ...
  /// SUB CATEGORY ITEMS
  UnmodifiableListView<SubCategoryItemModel> get getSubCategoryItems => UnmodifiableListView(_subCategoryItems);
  ...
  void fetchPopularCategories(){
    SocketService().once('fetchSubCategoryItems', (data){
      ZLoggerService.logOnInfo('FETCHING SUB CATEGORY ITEMS \n ---- $data ---- \n${DateTime.now()}');
      if(data != null) setSubCategoryItems = SubCategoryItemList.fromJson(data).list;
    });
  ...
  void _persistUserProductHistory() async{
    final _ = await _productRepository.persistUserProductHistory(_userProductHistory);
  }
}