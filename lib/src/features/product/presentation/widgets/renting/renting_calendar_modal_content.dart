...
class RentingCalendarModalContent extends StatefulWidget {
  ...
  @override
  State<RentingCalendarModalContent> createState() =>
      _RentingCalendarModalContentState();
}

class _RentingCalendarModalContentState
    extends State<RentingCalendarModalContent> {
  ...
  void initState() {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      final dates = context.read<ProductProvider>().getDates;
      _calendarController.selectedRange =
          PickerDateRange(dates['startDate'], dates['endDate']);
    });
    super.initState();
  }
            ...
            RentingCalendar(
              ...
              onSelectionChanged: (args) {
                if (args.value != null) {
                  ...
                  context.read<ProductProvider>().setDates =
                      _selectedDates.value ?? {};
                }
              },
              blackoutDates: (widget.product.unavailableBookingDates ?? [])
                  .map((obj) => DateTime.parse(obj.date ?? ''))
                  .toList(),
            ).expanded(),
                  ...
                  return Row(
                    ...
                    children: [
                      AppCard(
                        ...
                        child: Text(selectedDateValues?['startDate'] == null
                                ? 'Apr 16, 2024'
                            ...
                            .fontSize(14.0)
                            .regular()
                            .color(selectedDateValues?['startDate'] == null
                                ? kPrimaryWhite
                      ...
                      AppCard(
                        ...
                        child: Text(selectedDateValues?['endDate'] == null
                                ? 'Apr 16, 2024'
                                : ((selectedDateValues?['endDate'] ??
                                        DateTime.now()) as DateTime)
                                    .format('MMM dd, y'))
                            .fontSize(14.0)
                            .regular()
                            .color(selectedDateValues?['endDate'] == null
                                ? kPrimaryWhite
                                : const Color(0xFF101828))
                            .textScale(TextScaler.noScaling),
                      ).flexible(),
      ...
      bottomNavigationBar: AppBottomNavWrapper(
        child: Row(
          ...
          children: [
            ...
            AppPrimaryButton(
              onPressed: () {
                if (_selectedDates.value == null ||
                    _selectedDates.value?['startDate'] == null ||
                    _selectedDates.value?['endDate'] == null) {
                ...
                if (_selectedDates.value?['startDate']
                    .isAtSameMomentAs(_selectedDates.value?['endDate'])) {
                  duration = 1;
                } else {
                  duration = HelperUtil.getDuration(
                      _selectedDates.value?['startDate'],
                      _selectedDates.value?['endDate']); // Calculate duration
                }

                if (HelperUtil.getDurationPrice(
                        widget.product.prices ?? [], duration) ==
                    0) {
                  AppDialogUtil.showWarningAlert(
                      context, 'No price found for selected date range');
                  return;
                }
