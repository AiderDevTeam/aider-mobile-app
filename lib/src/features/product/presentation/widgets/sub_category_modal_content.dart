...
class SubCategoryModalContent extends StatefulWidget {
  const SubCategoryModalContent({super.key, required this.pageController,});

  final PageController pageController;

  @override
  State<SubCategoryModalContent> createState() => _SubCategoryModalContentState();
}
  ...
  Widget build(BuildContext context) {
    return Column(
      ...
      children: [
        ...
        const Text('SUB CATEGORIES').bold().color(kGrey700).fontSize(12.0).alignCenterLeft(),
        const VSpace(height: 20.0),
        BaseView<ProductViewModel>(
          builder: (context, productConsumer, child) {
            final subCategories = productConsumer.getSelectedCategory.subCategories?? [];
              ...
              itemBuilder: (context, index){
                ...
                return AppExpandable(
                  header: Container(
                    ...
                    child: Text(subCategory.name?? '').color(kGrey1200).medium().fontSize(14).height(20, 14).letterSpacing(-0.15),
                  ),
                    ...
                    child: ListView.builder(
                      ...
                      itemBuilder: (context, index){
                        ...
                        return Container(
                          ...
                          margin: index == (items.length-1)? null : EdgeInsets.symmetric(
                          ...
                          child: Text(item.name?? '').color(kGrey1200).medium().fontSize(14).height(20, 14).letterSpacing(-0.15).paddingSymmetric(horizontal: index == (items.length-1)? 16.0 : 0),
                        ).onPressed((){
