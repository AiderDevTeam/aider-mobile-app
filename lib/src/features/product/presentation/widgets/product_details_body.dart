...
class _ProductDetailsBodyState extends State<ProductDetailsBody> {
  ...
  void initState() {
    WidgetsBinding.instance.addPostFrameCallback((_) async {
      ...
      context
          ...
          .fetchProductUserDetail(uid: widget.product.userUid ?? '');
    });
    super.initState();
  }
  ...
  Widget build(BuildContext context) {
    return BaseView<ExploreViewProvider>(
        builder: (context, exploreProvider, child) {
      final productUser = exploreProvider.getProductUser;
      return AppScreenScaffold(
        bottomNavigationBar: AppBottomNavWrapper(
          child: AppPrimaryButton(
            onPressed: productUser == null
                ? null
                : () {
                    final user = context.read<UserProvider>().getUser;
                    if (user.uid == productUser.uid) {
                    ...
                    AppDialogUtil.showScrollableBottomSheet(
                      context: context,
                      builder: (context) =>
                          RentingModal(product: widget.product),
                    );
                  },
            ...
            children: [
              ...
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Row(
                      ...
                      children: [
                        Row(
                          children: [
                            (productUser?.hasProfilePhoto == false)
                                ? CircleAvatar(
                              ...
                              children: [
                                ...
                                Row(
                                  children: [
                                    ...
                                    Text('${productUser?.statistics?.vendorAverageRating ?? 0.0}')
                                        .regular()
                                        .fontSize(12.0)
                                        .color(kGrey700),
                                  ],
                  ...
                  AppNavigator.pushNamed(context, AppRoute.vendorProfileScreen,
                      arguments: productUser ?? const UserModel());
                }),
              ).paddingSymmetric(horizontal: kWidthPadding),
              const VSpace(height: 20.0),
              Container(
              ...
              BaseView<ReviewViewModel>(
                ...
                return Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text("${productUser?.displayName ?? ''}`s review")
                        .bold()
                        .fontSize(14.0)
                        .color(kGrey1200)
                        .overflowText(TextOverflow.ellipsis),
