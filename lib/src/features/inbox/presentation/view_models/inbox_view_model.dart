...
class InboxViewModel extends BaseViewModel{
  ...
  UnmodifiableListView<ChatModel> get getReceivedChats => UnmodifiableListView(_receivedChats);

  void fetchVendorConversations(){
    try{
      setLoading(true, component: 'receivedInbox');
      SocketService().once('fetchVendorConversation', (data){
        ZLoggerService.logOnInfo('FETCHING RECEIVED CONVERSATIONS \n ---- $data ---- \n${DateTime.now()}');
        if(data != null){
  ...
  UnmodifiableListView<ChatModel> get getSentChats => UnmodifiableListView(_sentChats);

  void fetchRenterConversations(){
    try{
      setLoading(true, component: 'sentInbox');
      SocketService().once('fetchRenterConversation', (data){
        ZLoggerService.logOnInfo('FETCHING SENT CONVERSATIONS \n ---- $data ---- \n${DateTime.now()}');
        if(data != null){
  ...
  UnmodifiableListView<MessageModel> get getMessages => UnmodifiableListView(_messages);
  void clearMessages(){
    _messages.clear();
  }
  ...
  void fetchMessages(){
    try{
      ...
      SocketService().on('fetchMessages', (data){
        ZLoggerService.logOnInfo('FETCHING MESSAGES \n ---- $data ---- \n${DateTime.now()}');

        if(data != null){
          if(data.runtimeType != List){
            final message = MessageModel.fromJson(data?? {});
            final index = _messages.indexWhere((obj) => obj.externalId == message.externalId && obj.conversationId == message.conversationId);
            if(index >= 0){
              _messages[index] = message;
            }else{
              final newIndex = _messages.indexWhere((obj) => obj.conversationId == message.conversationId);
  ...
  bool get canNudge => getMessages.isNotEmpty && getMessages.where((obj) => obj.type == kChatNudgeType).toList().length < 2;
  ...
  Future<void> approveBookingRequest(BuildContext context,{String? messageExternalId, required String bookingExternalId, required Map<String, dynamic> requestBody}) async{
    if(requestBody['status'] == 'canceled') AppDialogUtil.loadingDialog(context);
    if(requestBody['status'] == 'rejected') AppDialogUtil.loadingDialog(context);

    if(requestBody['status'] == 'accepted') setAcceptingRequestLoading = true;
    final result = await _inboxRepository.approveBookingRequest(bookingExternalId:bookingExternalId, requestBody: requestBody);
    if(context.mounted) {AppNavigator.pop(context);}

    result.fold((failure){
      if(requestBody['status'] == 'accepted') setAcceptingRequestLoading = false;
      ...
      final index = _messages.indexWhere((obj) => obj.externalId == messageExternalId);
      if(index >= 0){
  ...
  /// SEND NUDGE
  Future<void> sendNudge(BuildContext context,{String? conversationId, required Map<String, dynamic> requestBody}) async{
    AppDialogUtil.loadingDialog(context);
    final result = await _inboxRepository.sendNudge(conversationId: conversationId, requestBody: requestBody);
    if(context.mounted) {AppNavigator.pop(context);}
