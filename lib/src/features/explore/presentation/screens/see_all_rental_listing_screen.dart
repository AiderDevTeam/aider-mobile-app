...
class SeeAllRentalListeningScreen extends StatefulWidget {
  const SeeAllRentalListeningScreen({super.key, required this.param,});

  final Map<String, dynamic> param;

  @override
  State<SeeAllRentalListeningScreen> createState() => _SeeAllRentalListeningScreenState();
}

class _SeeAllRentalListeningScreenState extends State<SeeAllRentalListeningScreen> {

  Future<void> fetchProducts([int page = 1]) async {
    if (!mounted) return;
    await context.read<ExploreViewModel>().fetchProducts(
      context,
    ...
    return AppScreenScaffold(
      ...
      appBarActions: [
        ProfileActions(
          ...
          'fetchProductsByCategory' : 'products',
          categoryExternalId: widget.param['externalId'],
        ),
      ],
      child: (widget.param['type'] == 'category') ? BaseView<ExploreViewModel>(
          builder: (context, exploreConsumer, child) {
            if(exploreConsumer.getComponentLoading('fetchProductsByCategory') && exploreConsumer.getCategoryProducts.isEmpty){
              return const ProductLoadingEffect();
            }

            if(exploreConsumer.isComponentErrorType('fetchProductsByCategory')){
              return Center(
                child: ErrorResponseMessage(
                  message: exploreConsumer.componentErrorType?['error']?? '',
                  onRetry: () async{
                    await fetchProducts(exploreConsumer.getCategoryProductListCurrentPageNumber);
                  },
                ),
              );
            }
            ...
            return Column(
              ...
              children: [
                ...
                AppLoadMore(
                  ...
                  onLoadMore: () => fetchProducts(exploreConsumer.getCategoryProductListCurrentPageNumber),
                  isLoaderVisible: !exploreConsumer.getComponentLoading('fetchProductsByCategory'),
                  children: [
                    ProductGridView(
      ...
      BaseView<ExploreViewModel>(
        builder: (context, exploreConsumer, child) {
          if(exploreConsumer.getComponentLoading('products') && exploreConsumer.getProducts.isEmpty){
            return const ProductLoadingEffect();
          }
          ...
          if(exploreConsumer.isComponentErrorType('products')){
            return Center(
              child: ErrorResponseMessage(
                message: exploreConsumer.componentErrorType?['error']?? '',
                onRetry: () async{
                  await fetchProducts(exploreConsumer.getProductsPageNumber);
                },
              ),
            );
          }
          ...
          return Column(
            ...
            children: [
              ...
              AppLoadMore(
                ...
                onLoadMore: () => fetchProducts(exploreConsumer.getProductsPageNumber),
                isLoaderVisible: !exploreConsumer.getComponentLoading('products'),
                children: [
                  ProductGridView(
