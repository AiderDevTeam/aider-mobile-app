...
class SocketService{
  ...
  void connect(String userExternalId, dynamic Function(dynamic) handler){
    _socket = IO.io(RemoteConfigService.getRemoteData.socketBaseUrl,
        IO.OptionBuilder().setTransports(['websocket'])
            .setQuery({
              'userExternalId': userExternalId,
              'externalIdType':'user',
            }).setExtraHeaders({'authorization': RemoteConfigService.getRemoteData.socketKey})
            .build(),
    );
  ...
  void onDisconnect(){
    _socket.onDisconnect((data) => ZLoggerService.logOnInfo('SOCKET DISCONNECTED....'));
  }
